...
for i=2:nbins+1    
    ...
    y_pre_merger = out_none.y_j(idx);    
    y_post_merger = out_merge.y_j(idx);    
    y_post_merger5 = out_merge5.y_j(idx);
    y_post_merger10 = out_merge10.y_j(idx);
    
    prob_loss(1, i-1) = 100*(sum(y_post_merger < y_pre_merger)./size(y_pre_merger,2));
    prob_loss(2, i-1) = 100*(sum(y_post_merger5 < y_pre_merger)./size(y_pre_merger,2));
    prob_loss(3, i-1) = 100*(sum(y_post_merger10 < y_pre_merger)./size(y_pre_merger,2));
    
    idx_loss = logical(y_post_merger < y_pre_merger);
    idx_loss5 = logical(y_post_merger5 < y_pre_merger);
    idx_loss10 = logical(y_post_merger10 < y_pre_merger);
    
    % 0 percent
    y_median(1,i-1) = prctile(100*(log(y_post_merger(idx_loss)) - log(y_pre_merger(idx_loss))), 50);
    y_25(1,i-1) = prctile(100*(log(y_post_merger(idx_loss)) - log(y_pre_merger(idx_loss))), 2.5);
    y_975(1,i-1) = prctile(100*(log(y_post_merger(idx_loss)) - log(y_pre_merger(idx_loss))), 97.5);
    
    % 5 percent
    y_median(2,i-1) = prctile(100*(log(y_post_merger5(idx_loss5)) - log(y_pre_merger(idx_loss5))), 50);
    y_25(2,i-1) = prctile(100*(log(y_post_merger5(idx_loss5)) - log(y_pre_merger(idx_loss5))), 2.5);
    y_975(2,i-1) = prctile(100*(log(y_post_merger5(idx_loss5)) - log(y_pre_merger(idx_loss5))), 97.5);
    
    % 10 percent
    y_median(3,i-1) = prctile(100*(log(y_post_merger10(idx_loss10)) - log(y_pre_merger(idx_loss10))), 50);
    y_25(3,i-1) = prctile(100*(log(y_post_merger10(idx_loss10)) - log(y_pre_merger(idx_loss10))), 2.5);
    y_975(3,i-1) = prctile(100*(log(y_post_merger10(idx_loss10)) - log(y_pre_merger(idx_loss10))), 97.5);
...
ylabel('Probability of output loss ($\%$)', 'FontSize', font, 'interpreter', 'latex');
title('A. Probability of market level output loss','FontSize', font, 'interpreter', 'latex');
...
subplot(1, 2, 2);
errorbar(bins_means, y_median(2,:), abs(y_median(2,:)-y_25(2,:)), abs(y_median(2,:)-y_975(2,:)), 'x', 'MarkerSize', ms, 'LineWidth', lw, 'Color', rgb('DarkBlue'), 'MarkerEdgeColor', rgb('DarkBlue'),'MarkerFaceColor', rgb('DarkBlue'));
hold on;
errorbar(bins_means, y_median(3,:), abs(y_median(3,:)-y_25(3,:)), abs(y_median(3,:)-y_975(3,:)), 'o', 'MarkerSize', ms, 'LineWidth', lw, 'Color', rgb('DarkRed'), 'MarkerEdgeColor', rgb('DarkRed'),'MarkerFaceColor', rgb('DarkRed'));
xlabel('Joint initial market share of merging firms', 'FontSize', font, 'interpreter', 'latex');
ylabel('Output loss ($\%$)', 'FontSize', font, 'interpreter', 'latex');
legend('$\Delta = 5 \%$', '$\Delta = 10 \%$', 'FontSize', font, 'interpreter', 'latex', 'Location', 'SouthWest')
title('B. Market level output loss','FontSize', font, 'interpreter', 'latex');
set(gca,'ticklabelinterpreter','latex');
xtickformat('%3.2f')
ax = gca;
ax.FontSize = font;
...
saveas(gcf,'../results/figures/figd2_output_losses.png');
